...
public class HapiParser : IHL7Parser
{
    ...
    public string CreateAck(string originalMessage)
    {
        ...
        try
        {
            ...
            // Pega uma referência direta para o segmento MSH do nosso objeto ACK para facilitar o acesso.
            var msh = ack.MSH;

            // ######################################################
            // # CORREÇÃO DEFINITIVA DO BUG DE FORMATAÇÃO           #
            // ######################################################
            // Forçamos a definição dos caracteres de separação no objeto MSH.
            // Isso resolve um bug conhecido no encoder do PipeParser que pode corromper a mensagem
            // ao não encontrar estes campos preenchidos.
            // MSH-1: Define o separador de campos.
            msh.FieldSeparator.Value = "|";
            // MSH-2: Define os outros caracteres de codificação.
            msh.EncodingCharacters.Value = "^~\\&";

            // --- Continuação da Construção do MSH ---
    ...
    {
        // O bloco 'try' garante que o método não quebre mesmo se a mensagem original for inválida.
        try
        {
            // 1. Parse e preparação, similar ao CreateAck.
            var original = _parser.Parse(originalMessage);
            var terserOriginal = new Terser(original);
            var nak = new ACK(); // A estrutura base de um NAK é um ACK.

            // 2. Construindo o Segmento MSH
            var msh = nak.MSH;

            // ######################################################
            // # APLICANDO A MESMA CORREÇÃO DO BUG NO NAK           #
            // ######################################################
            // Forçamos a definição dos caracteres de separação para evitar o bug de formatação.
            msh.FieldSeparator.Value = "|";
            msh.EncodingCharacters.Value = "^~\\&";
            ...
            // 3. Construindo o Segmento MSA (a parte que define o NAK)
            var msa = nak.MSA;
            // MSA-1: "AE" (Application Error) é o código que define esta mensagem como uma rejeição/erro.
            msa.AcknowledgmentCode.Value = "AE";
            // MSA-2: O ID da mensagem original que estamos rejeitando.
            msa.MessageControlID.Value = terserOriginal.Get("MSH-10-1");
            // MSA-3: O campo de texto livre onde colocamos a descrição do erro.
            msa.TextMessage.Value = errorMessage;

            // 4. Codifica e Retorna
            return _parser.Encode(nak);
        }
        // 'catch' como plano B para mensagens totalmente corrompidas.
        catch (Exception)
        {
